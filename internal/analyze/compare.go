...
func buildWindow(data *Loaded, start, end cache.Month, label string, ci config.CodeImpactConfig) WindowMetrics {
	monthly := rollupMonthly(data, start, end, ci)
	weekly := rollupWeekly(data, start, end, ci)
	...
	totals.CodeImpact = computeCodeImpact(totals.UniqueFilesTouched, totals.LOCAdded+totals.LOCDeleted, totals.PRsMerged, ci)
...
func currentWindow(data *Loaded, currentMonth cache.Month, length int, ci config.CodeImpactConfig) WindowMetrics {
	if length < 1 {
		length = 1
	}
	start := currentMonth.Add(-(length - 1))
	return buildWindow(data, start, currentMonth, "Current", ci)
}

// priorWindow is the same-length window immediately preceding the current.
// e.g., current = 2026-01..2026-04 → prior = 2025-09..2025-12.
func priorWindow(data *Loaded, current WindowMetrics, ci config.CodeImpactConfig) WindowMetrics {
	length := current.Window.LengthMonths
	curStart, _ := cache.ParseMonth(current.Window.Start)
	end := curStart.Add(-1)
	start := end.Add(-(length - 1))
	return buildWindow(data, start, end, "Prior", ci)
}

// yoyWindow is the same calendar months, one year earlier.
// e.g., current = 2026-01..2026-04 → yoy = 2025-01..2025-04.
func yoyWindow(data *Loaded, current WindowMetrics, ci config.CodeImpactConfig) WindowMetrics {
	curStart, _ := cache.ParseMonth(current.Window.Start)
	curEnd, _ := cache.ParseMonth(current.Window.End)
	start := curStart.Add(-12)
	end := curEnd.Add(-12)
	return buildWindow(data, start, end, "YoY", ci)
}
...
func lastQuarters(data *Loaded, currentMonth cache.Month, n int, ci config.CodeImpactConfig) []WindowMetrics {
	if n < 1 {
		return nil
	}
	...
	for i := n - 1; i >= 0; i-- {
		...
		out = append(out, buildWindow(data, start, end, q.label(), ci))
	}
	return out
}
...
func fullHistory(data *Loaded, backfillStart, currentMonth cache.Month, ci config.CodeImpactConfig) HistoryRollup {
	return HistoryRollup{Monthly: rollupMonthly(data, backfillStart, currentMonth, ci)}
}
