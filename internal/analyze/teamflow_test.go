...
func TestDeriveTeamFlowMonthlyBucketing(t *testing.T) {
	data := &Loaded{
		Issues: []cache.JiraIssue{
			// created Jan, resolved Feb, Claude-labeled, 5 SP, 48h cycle
			{Created: mustTime("2026-01-05T00:00:00Z"), Resolved: ptrTime(mustTime("2026-02-10T00:00:00Z")),
				Labels: []string{"CLAUDE_GEN"}, StoryPoints: 5, CycleHours: 48},
			// created Feb, resolved Feb, not Claude, 96h cycle
...
func TestDeriveClaudeCut(t *testing.T) {
	data := &Loaded{Issues: []cache.JiraIssue{
		{Resolved: ptrTime(mustTime("2026-02-10T00:00:00Z")), Labels: []string{"CLAUDE_GEN"}, CycleHours: 50},
		{Resolved: ptrTime(mustTime("2026-02-12T00:00:00Z")), Labels: []string{"CLAUDE_GEN"}, CycleHours: 70},
		{Resolved: ptrTime(mustTime("2026-02-15T00:00:00Z")), CycleHours: 200},
		{Resolved: ptrTime(mustTime("2026-09-01T00:00:00Z")), Labels: []string{"CLAUDE_GEN"}, CycleHours: 999}, // out of window
	}}
